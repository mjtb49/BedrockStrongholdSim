package kaptainwutax.featureutils.Util;
...
public class BedrockRandom extends Random {
    ...
    public void checkForChange(long seed,int offset) {
        this.setSeed(seed);
        for(int i = 0; i < 760+offset; i++) {
            this.nextFloat();
        }
        int failcount = 0;
        for (int i = 0; i < 12;i++) {
            if (this.nextFloat() < 0.9)
                failcount++;
            if (failcount > 3)
                return;
        }
        System.out.println(seed + " "+(12 - failcount));
    }

    public static void main(String[] args) {
        double start = System.currentTimeMillis();
        final long numThreads = 4;
        final long maxSeed = (1L << 32);
        LongStream.range(0, numThreads).parallel().forEach(i -> {
            BedrockRandom r = new BedrockRandom();
            long step = maxSeed/numThreads;
            long bound = (i+1 == numThreads) ? maxSeed : (i+1)*step;
            i *= step;
            for (; i < bound; i++) {
                r.checkForChange(i,1);
            }
        });
        //BedrockRandom r = new BedrockRandom();
        //r.checkForChange((4001523836L << 32 >>> 32));
    }//
